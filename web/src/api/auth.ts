...
export async function login(
  ...
  const { data } = await client.post<AuthTokens>("/api/auth/login", {
    email,
    password,
  });
  return data;
...
export async function setup(
  ...
  const { data } = await client.post<AuthTokens>("/api/auth/setup", {
    email,
    password,
    name,
  });
...
export async function register(
  ...
  const { data } = await client.post<AuthTokens>("/api/auth/register", {
    username,
    password,
    name,
  });
...
export async function refresh(refreshToken: string): Promise<AuthTokens> {
  const { data } = await client.post<AuthTokens>("/api/auth/refresh", {
