...
export function EndpointDetail() {
  ...
  const duplicateEndpoint = useMutation({
    mutationFn: () => endpointsApi.duplicate(id!),
    onSuccess: (ep) => {
      qc.invalidateQueries({ queryKey: ["endpoints"] });
  ...
  const createRule = useMutation({
    mutationFn: () =>
      endpointsApi.createRule(id!, {
        type: ruleType,
        threshold:
          ruleType === "latency_gt" ? Number(ruleThreshold) : undefined,
  ...
  if (endpointError) {
    return (
      <div className={styles.page}>
        <ErrorBanner
          message={getErrorMessage(endpointError)}
          onRetry={() => refetch()}
        />
      </div>
    );
      ...
      <div className={`card ${styles.headerCard}`}>
        ...
        <div className={styles.headerMeta}>
          <div className={styles.metaItem}>
            ...
            <span
              style={{
                color: endpoint.uptime_24h < 90 ? "var(--red)" : "var(--green)",
              }}
            >
          ...
          <div className={styles.metaItem}>
            ...
            <span
              style={{
                color:
                  threshold && endpoint.avg_latency > threshold
                    ? "var(--yellow)"
                    : "var(--text-1)",
              }}
            >
        ...
        <div className={styles.headerActions}>
          <RoleGuard role="admin">
            ...
            >
              {checkNow.isPending ? <span className="spinner" /> : "▶"}{" "}
              {t("detail.checkNow")}
            </button>
            <button
              className="btn btn-ghost btn-sm"
            ...
            <button
              className="btn btn-ghost btn-sm"
              onClick={() => duplicateEndpoint.mutate()}
              disabled={duplicateEndpoint.isPending}
            >
              {duplicateEndpoint.isPending ? (
        ...
        <div className={styles.statsGrid}>
          <div className="card">
            <div className={styles.stat}>
              <span className={styles.statLabel}>{t("detail.p50")}</span>
              <span className={styles.statValue}>
                {Number(stats.p50_latency).toFixed(2)}ms
              </span>
            </div>
          </div>
          <div className="card">
            <div className={styles.stat}>
              <span className={styles.statLabel}>{t("detail.p95")}</span>
              <span className={styles.statValue}>
                {Number(stats.p95_latency).toFixed(2)}ms
              </span>
            </div>
          </div>
          <div className="card">
            <div className={styles.stat}>
              <span className={styles.statLabel}>{t("detail.incidents")}</span>
              <span
                className={styles.statValue}
                style={{
                  color:
                    stats.incidents_7d > 0 ? "var(--red)" : "var(--text-1)",
                }}
              >
                {stats.incidents_7d}
              </span>
          ...
          <div className="card">
            <div className={styles.stat}>
              <span className={styles.statLabel}>
                {t("detail.checksToday")}
              </span>
              <span className={styles.statValue}>{stats.checks_today}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="card">
            ...
            <div className={styles.addRuleRow}>
              ...
              {ruleType === "latency_gt" && (
                <input
                  ...
                  style={{ width: 120 }}
                  type="number"
                  placeholder={t("rule.msThreshold")}
                  value={ruleThreshold}
              ...
              {ruleType === "down" && (
                <input
                  ...
                  style={{ width: 140 }}
                  type="number"
                  placeholder={t("rule.consecutiveFails")}
                  value={ruleFails}
                  onChange={(e) => setRuleFails(e.target.value)}
                />
              )}
              <button
        ...
        {rules.map((rule) => (
          <AlertRuleItem
            key={rule.id}
            rule={rule}
            onDelete={(rid) => deleteRule.mutate(rid)}
          />
        ))}

        {rules.length === 0 && !addRuleOpen && (
          <p style={{ padding: "16px", color: "var(--text-3)", fontSize: 12 }}>
            {t("detail.noRules")}
          </p>
        )}
      </div>
